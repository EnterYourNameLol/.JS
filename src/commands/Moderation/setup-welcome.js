...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(!interaction.guild.members.me.permissions.has(PermissionFlagsBits.SendMessages)) {
            interaction.reply({content: "I don't have permissions for this.", ephemeral: true}).catch((err) => {
                console.log(err)
                });
        }
        ...
        welcomeSchema.findOne({Guild: interaction.guild.id}, async (err, data) => {
            ...
            interaction.reply({content: 'Succesfully created a welcome message', ephemeral: true}).catch((err) => {
                console.log(err)
                });
        })
    }
